...
# ==================================================
    ...
    if {"Lat", "Long"}.issubset(df_view.columns):
        ...
        if frame_values:
            ...
            # Initialize session state for spatial animation
            if "spatial_frame_index" not in st.session_state:
                st.session_state["spatial_frame_index"] = 0
            if "spatial_playing" not in st.session_state:
                st.session_state["spatial_playing"] = False
            if "spatial_animation_counter" not in st.session_state:
                st.session_state["spatial_animation_counter"] = 0
            
            # Custom Play/Pause buttons for Spatial map
            st.markdown("**Animation Controls:**")
            col1, col2, col3 = st.columns(3)
            
            with col1:
                if st.button("▶ Play Spatial Animation", key="play_spatial"):
                    st.session_state["spatial_playing"] = True
                    st.session_state["spatial_animation_counter"] += 1
            
            with col2:
                if st.button("⏸ Pause Spatial Animation", key="pause_spatial"):
                    st.session_state["spatial_playing"] = False
            ...
            if st.session_state.get("spatial_playing"):
                st.session_state["spatial_frame_index"] = (st.session_state["spatial_frame_index"] + 1) % len(frame_values)
                time.sleep(0.7)  # Match Temporal animation speed
                st.rerun()
            ...
            current_frame_idx = st.slider(
                "Select Time Frame",
                min_value=0,
                max_value=len(frame_values) - 1,
                value=st.session_state.get("spatial_frame_index", 0),
                key="spatial_frame_slider"
            )
            st.session_state["spatial_frame_index"] = current_frame_idx
            
            # Display current frame
            current_frame_value = frame_values[current_frame_idx]
            current_frame_df = df_view[df_view["Date_Frame"] == current_frame_value]
            current_traces = build_traces(current_frame_df)
            
            # Create figure with current frame data
            fig4 = go.Figure(data=current_traces)
            
            # Add slider annotation showing date and risk
            current_risk = frame_risk_map.get(current_frame_value, 0.0)
            fig4.add_annotation(
                text=f"<b>Date: {current_frame_value} | Risk: {current_risk:.2f}</b>",
